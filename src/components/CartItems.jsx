    ...
    <article key={cartID} className='mb-12 flex flex-col gap-y-4 sm:flex-row flex-wrap border-b border-base-300 pb-6 last:border-b-0'>
      ...
      <div className='sm:ml-24'>
        ...
        <div className='form-control max-w-xs'>
          ...
          <select name="amount" id="amount" className='mt-2 select select-bordered select-xs select-primary' value={amount} onChange={handleAmount}>
            {generateAmountOptions(amount +5)}
          </select>
        </div>
        <button className='mt-2 link link-primary link-hover text-sm' onClick={removeItemInCart}>remove</button>
      </div>
